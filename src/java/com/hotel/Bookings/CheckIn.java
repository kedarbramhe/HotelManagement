...
import com.vedisoft.documents.Documents;
import com.vedisoft.documents.DocumentsDAO;
...
public class CheckIn extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            out.println("<form name=\"thisform\" action=\"DocumentAdd\">");
            ...
            ArrayList<Integer> ab = bd.findAllTodaysCheckin(curr);
            for (Integer b : ab) {
                Bookings b1 = bd.find(b.intValue());
                Documents d = dd.findbyCurrent(b1.getBookingId());
                if (d.getGuestId() == 0) {
                    Guest g = gd.find(b1.getGuestId());
                    out.println("<option name=\"" + g.getGuestName() + "\" value=\"" + g.getGuestId() + "\">" + g.getGuestName() + "</option>");
                }

            }
            out.println("<input type=\"submit\" value=\"Checkin\">");
            out.println("</select>");
            out.println("</form>");
            out.println("</div></div>");
